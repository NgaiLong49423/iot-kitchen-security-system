...
> **Phiên bản tài liệu:** 0.7.0
> **Trạng thái:** Bản checkpoint khóa phạm vi demo cuối: loại BLE/WiFi scanning, bổ sung heartbeat, phát hiện suy giảm nghiêm trọng sau phá hoại và escalation có xác nhận
> **Ngày tạo:** 2026-06-26
...
Bản 0.7.0 bổ sung chuỗi xử lý phá hoại hai giai đoạn:
...
ESP32-S3 phải gửi heartbeat đến Google Apps Script mỗi 10 giây trong cấu hình demo. Gửi heartbeat thất bại không được chặn vòng lặp cảm biến quá lâu.

### BR-97. Quy tắc heartbeat timeout

Apps Script chỉ xem thiết bị mất liên lạc khi không nhận heartbeat trong khoảng cấu hình 30–60 giây. Một request lỗi đơn lẻ không đủ để kết luận offline.
...
1. Hệ thống theo dõi `ldr_covered` và thời gian `object_near` kéo dài.
...
Khi LDR bị che hoặc vật áp sát đủ thời gian, hệ thống báo sabotage ngay cả khi `alarm_enabled = false`, sau đó vẫn tiếp tục theo dõi để có thể nâng lên DS-03.
...
Heartbeat timeout phải có khoảng đệm đủ lớn so với chu kỳ gửi để tránh báo offline do một request lỗi. Giá trị timeout tối thiểu phải lớn hơn hoặc bằng ba lần chu kỳ heartbeat.
...
Chu kỳ heartbeat demo phải từ 5–30 giây; timeout phải từ 30–60 giây và không nhỏ hơn ba lần chu kỳ gửi.
...
| Heartbeat | Board gửi 10 giây/lần; timeout 30–60 giây |
...
```text
DS-04:
    phát hiện LDR bị che hoặc vật áp sát kéo dài
