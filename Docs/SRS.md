...
> **Phiên bản tài liệu:** 0.6.6
> **Trạng thái:** Bản checkpoint cập nhật BLE nhận diện chủ nhà, SOS escalation và weighted intrusion score
> **Ngày tạo:** 2026-06-26
...
Bản 0.6.6 kế thừa phạm vi nghiệp vụ của checkpoint 0.6.5: tập trung vào chống trộm cho một khu vực giám sát hiện tại là phòng bếp; không đưa lại báo cháy, Flame Sensor, đo nhiệt độ bếp hoặc RFID-RC522 vào phạm vi chính. Thay đổi chính của bản này là cập nhật DS-03/FR-04 theo logic `weighted intrusion score`: PIR và HY-SRF05/object-near là tín hiệu xác nhận mạnh, còn LDR và `night_mode` là tín hiệu bối cảnh hỗ trợ. Cách tính này giúp giảm báo động giả khi chỉ có chuyển động kèm thay đổi ánh sáng, đồng thời vẫn kích hoạt cảnh báo đầy đủ khi có đủ bằng chứng vật lý.
...
| Trạng thái hiện tại | Bản checkpoint 0.6.6; giữ board chính `Freenove ESP32-S3 WROOM + Camera OV3660`, bổ sung BLE trusted device scanning, SOS authority escalation qua Google Apps Script và cập nhật logic weighted intrusion score cho DS-03/FR-04 |
...
Hệ thống phải cho phép Child hoặc Parent/Admin kích hoạt SOS khẩn cấp qua dashboard. Sau khi Parent/Admin xác nhận, Google Apps Script phải gửi email escalation kèm địa chỉ nhà và ghi chú bổ sung đến contact cơ quan chức năng/công an hoặc contact mô phỏng được cấu hình hợp lệ.
...
8. Nếu ít nhất một Parent/Admin xác nhận qua email trong vòng 60 giây, Google Apps Script gửi email escalation đến contact được cấu hình hợp lệ.
9. Email escalation phải chứa địa chỉ nhà đã cấu hình, nguồn SOS, thời điểm SOS, trạng thái hệ thống và `sos_authority_note` nếu có.
10. Trong demo, contact này là email phụ của thành viên nhóm và đóng vai trò cơ quan chức năng mô phỏng; trong triển khai thật, contact chỉ được cấu hình khi hợp lệ.
11. Parent/Admin dùng `reset_alarm` để tắt còi/LED và kết thúc cảnh báo SOS.

#### Ràng buộc

Hệ thống chỉ gửi authority escalation sau khi Parent/Admin xác nhận. Hệ thống không đảm bảo thời gian phản hồi hoặc thời gian công an/cơ quan chức năng đến hiện trường.

#### Acceptance Scenario

Khi `sos_child = true`, hệ thống phải bật còi/LED, đặt `alarm_status = SOS_ALERT`, gửi notification cho Parent/Admin, yêu cầu Google Apps Script gửi email xác nhận escalation và sau xác nhận phải gửi email escalation có địa chỉ nhà/ghi chú đến contact đã cấu hình.
...
| SOS Escalation | Bấm link xác nhận trong email | Gửi email authority escalation có địa chỉ nhà và ghi chú bổ sung đến contact được cấu hình |
