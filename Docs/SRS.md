...
21. Board chính của checkpoint hiện tại là `Freenove ESP32-S3 WROOM + Camera OV3660`; không được mặc định dùng lại pinout, board profile hoặc cấu hình upload của ESP32-CAM/AI Thinker cũ hoặc board ESP32-S3 CAM OV2640 trước đó nếu chưa kiểm tra lại theo module mới.
22. Không được hiểu `unknown_wifi_detection_enabled = false` là loại bỏ chức năng WiFi/MAC lạ khỏi phạm vi. Đây là công tắc kiểm soát nhiễu môi trường demo; khi bật lại, chức năng phải chạy theo rule đã chốt.
23. Nếu phát hiện thiết bị đáng tin chưa ổn định trong môi trường demo, được phép dùng controlled demo input qua Admin Dashboard để cập nhật `known_device_present`, nhưng phải giữ nguyên logic re-arm và `system_armed`.
24. ESP32-S3 được phép dùng Bluetooth Low Energy (BLE) scanning tích hợp để nhận diện điện thoại/thiết bị đáng tin của chủ nhà; không được giả định cần thêm module Bluetooth rời nếu board hiện tại đã hỗ trợ BLE.
25. BLE trusted device chỉ được dùng để cập nhật `trusted_ble_present`, `trusted_device_source` và/hoặc `known_device_present`; nếu cảnh báo đang active, BLE không được tự động xóa cảnh báo hiện tại.
26. SOS authority escalation phải đi qua Google Apps Script, có xác nhận của Parent/Admin trước khi gửi thông tin ra contact cơ quan chức năng/công an được cấu hình.
27. Địa chỉ nhà, contact cơ quan chức năng/công an và các thông tin nhạy cảm của escalation phải được cấu hình bảo mật trong Google Apps Script hoặc cấu hình triển khai, không hardcode lộ trong dashboard dành cho Child.
28. Hệ thống được thiết kế để gửi thông tin SOS nhanh và đầy đủ nhất có thể, nhưng không được cam kết hoặc bảo đảm thời gian phản hồi/thời gian công an đến hiện trường.
...
| BLE | Bluetooth Low Energy, Bluetooth tiết kiệm năng lượng dùng để phát hiện thiết bị đáng tin ở gần |
| BLE Scanning | Việc ESP32-S3 quét tín hiệu BLE xung quanh để phát hiện điện thoại/tag đáng tin của chủ nhà |
| Active BLE Scan | Kiểu quét BLE chủ động; ESP32-S3 không chỉ nghe gói advertising chính mà còn yêu cầu thêm scan response để đọc các trường như `Complete Local Name` nếu app đặt tên ở đó |
| BLE Advertiser App | Ứng dụng bên thứ ba trên điện thoại dùng để phát beacon BLE phục vụ demo/nhận diện trusted phone |
| Advertising Data | Gói dữ liệu BLE phát chính, thường chứa `Flags` và một số trường ngắn |
| Scan Response Data | Gói dữ liệu bổ sung trả về khi thiết bị quét dùng active scan; trong demo hiện tại chứa `Complete Local Name` của trusted phone |
| Complete Local Name | Tên đầy đủ của thiết bị BLE trong gói advertising hoặc scan response; bản demo dùng tên như `Mobile_Phone_Number1` để whitelist trusted phone |
| Freenove ESP32-S3 WROOM | Module ESP32-S3 WROOM của Freenove, dùng làm board điều khiển chính trong checkpoint hiện tại |
| OV3660 | Cảm biến camera dùng để chụp ảnh trong phiên bản phần cứng hiện tại |
| Pinout | Sơ đồ chân của board/module, dùng để xác định chân nguồn, GND, GPIO và chân camera |
| Controlled Demo Input | Đầu vào demo có kiểm soát, tức Admin có thể đặt giá trị để chứng minh logic hệ thống khi môi trường thật không ổn định |
| MAC Randomization | Cơ chế điện thoại/laptop tự đổi hoặc ẩn MAC để tăng quyền riêng tư, có thể làm phát hiện thiết bị đáng tin/lạ kém ổn định |
...
| Trusted BLE Device | Thiết bị BLE đáng tin, ví dụ điện thoại chủ nhà, BLE tag hoặc ESP32 phụ phát beacon được whitelist |
| Trusted Phone | Điện thoại chủ nhà được dùng làm thiết bị định danh chính trong phạm vi dự án |
...
| Authority Escalation | Luồng gửi thông tin SOS đã xác nhận đến contact cơ quan chức năng/công an hoặc contact mô phỏng được cấu hình trong Google Apps Script |
| ACK | Acknowledgement, xác nhận đã nhận cảnh báo; ACK không đồng nghĩa với reset báo động |
| Emergency Note | Ghi chú bổ sung của Parent/Admin gửi kèm email SOS escalation để mô tả tình hình hiện tại hoặc nhắc nhở cơ quan tiếp nhận |
...
| FR-10 | SOS khẩn cấp qua Dashboard và authority escalation qua Google Apps Script | Đã phân rã bản demo mở rộng |
...
| FR-13 | Chế độ demo | Demo Mode chỉ hỗ trợ mức cần thiết; không làm giảm phạm vi chức năng chính |
...
| Camera | OV3660 |
| Bộ nhớ module | Phụ thuộc đúng biến thể Freenove ESP32-S3 WROOM thực tế; SRS không giữ giả định N16R8 của board cũ |
| Vai trò trong hệ thống | Điều khiển chính, kết nối WiFi/Arduino Cloud, xử lý camera OV3660 và điều phối cảm biến |
...
| HY-SRF05 / HC-SR04-compatible ultrasonic sensor | Đo khoảng cách vật thể |
...
| BLE scanning tích hợp trên ESP32-S3 | Quét điện thoại/tag BLE đáng tin để nhận diện chủ nhà gần khu vực; với điện thoại dùng app BLE advertiser bên thứ ba, ESP32-S3 phải dùng active scan nếu tên nằm trong scan response |
| Google Apps Script hoặc dịch vụ tương đương | Gửi email xác nhận SOS, gửi authority escalation sau khi Parent/Admin xác nhận, ghép địa chỉ nhà và ghi chú bổ sung |
| App BLE advertiser trên điện thoại | Phát beacon BLE với `Complete Local Name` đã whitelist, ví dụ `Mobile_Phone_Number1`, để ESP32-S3 nhận diện trusted phone trong demo |
...
Trong tài liệu này, `MVP` không được hiểu là cắt bớt chức năng để làm bản đơn giản. `MVP` ở checkpoint hiện tại nghĩa là **bản tối thiểu nhưng phải chạy đủ các luồng chính để nộp và demo trước hội đồng**.

Các nhóm chức năng sau thuộc phạm vi bắt buộc của bản demo hoàn chỉnh:

| Nhóm | Bắt buộc trong bản demo hoàn chỉnh | Ghi chú kiểm soát triển khai |
|---|---|---|
| Cảm biến vật lý | PIR, cảm biến siêu âm HY-SRF05 / HC-SR04-compatible, LDR, DS1307 RTC | Không giả lập nếu chưa có xác nhận riêng |
| Cảnh báo tại chỗ | Buzzer, LED đỏ, LED xanh | LED đỏ được phép nháy bằng logic nội bộ, không cần Cloud Variable riêng cho từng nhịp nháy |
| Arduino Cloud | Thing, biến Cloud, dashboard Child/Parent/Admin | Dashboard phải đủ thao tác demo và debug |
| Camera | Chụp ảnh thủ công và tự chụp 1 ảnh khi có `intrusion_alert` mới | Dùng camera OV3660 trên board Freenove ESP32-S3 WROOM |
| Notification | Telegram/notification cho intrusion, SOS, sabotage, lỗi lịch, lỗi camera và unknown WiFi đủ điều kiện | Phải có cooldown để tránh spam |
| SOS | SOS Child, SOS Adult, email confirmation/escalation qua Google Apps Script, gửi địa chỉ nhà và ghi chú bổ sung sau khi Parent/Admin xác nhận | Demo mặc định dùng contact mô phỏng; triển khai thật chỉ gửi đến contact cơ quan chức năng/công an hợp lệ đã cấu hình |
| Anti-sabotage | Phát hiện LDR bị che hoặc vật thể quá gần đủ thời gian | Hoạt động độc lập với `alarm_enabled` |
| Lịch tự động | Auto arm/disarm bằng RTC | Không dùng Internet/Cloud time làm nguồn thời gian chính |
| WiFi/MAC lạ | Có phát hiện dạng pre-warning và có công tắc kiểm soát nhiễu | Không được tự kích hoạt `intrusion_alert` độc lập |
| Thiết bị đáng tin | Có logic `known_device_present`, re-arm countdown, `DISARMED_BY_TRUSTED_DEVICE` và BLE trusted device scanning | Ưu tiên BLE scan bằng ESP32-S3; có thể dùng controlled demo input nếu phát hiện thật không ổn định |

Các chức năng trên có thể được triển khai theo mức demo ổn định, nhưng không được xóa khỏi phạm vi nếu không có quyết định thay đổi SRS mới.
...
Trong checkpoint 0.6.8, nguồn phát hiện thiết bị đáng tin ưu tiên là Bluetooth Low Energy (BLE) scanning trên ESP32-S3. Board chính quét điện thoại chủ nhà, BLE tag hoặc ESP32 phụ phát beacon đã được whitelist để cập nhật `trusted_ble_present` và/hoặc `known_device_present`.

Đối với điện thoại trong bản demo hiện tại, phương án đã chốt là dùng app BLE advertiser bên thứ ba. Điện thoại phải phát tên trusted phone dạng `Mobile_Phone_Number1`, `Mobile_Phone_Number2`, `Mobile_Phone_Number3` hoặc `Mobile_Phone_Number4`. Nếu app đặt `Complete Local Name` trong `Scan Response data`, firmware ESP32-S3 phải dùng active BLE scan để đọc được tên này. Hệ thống không dùng MAC address điện thoại hoặc Bluetooth pairing làm định danh chính vì chúng không ổn định cho mục tiêu demo.

Nếu thiết bị đáng tin xuất hiện, hệ thống không báo đột nhập cho khu vực đó, nhưng không được tự động xóa cảnh báo đang active nếu cảnh báo đã xảy ra trước đó.
...
### 3.6 SOS authority escalation phải có xác nhận và cấu hình hợp lệ

SOS là chức năng khẩn cấp, nhưng hệ thống không được tự động gửi thông tin đến contact cơ quan chức năng/công an nếu chưa có xác nhận của Parent/Admin.

Sau khi có xác nhận, Google Apps Script được phép gửi email escalation chứa địa chỉ nhà đã cấu hình, loại sự kiện SOS, thời gian, trạng thái hệ thống và ghi chú bổ sung của Parent/Admin đến contact được cấu hình.
...
| Google Apps Script SOS Escalation | Hệ thống phải gọi Apps Script để tạo email xác nhận, nhận trạng thái ACK, gửi email escalation kèm địa chỉ nhà và ghi chú bổ sung khi Parent/Admin xác nhận. |
| Ứng dụng Dashboard | Hệ thống phải cho phép tương tác theo vai trò qua widget dashboard. |
| App BLE advertiser trên điện thoại | Điện thoại trusted phải phát BLE beacon có `Complete Local Name` đúng whitelist; app scanner chỉ dùng để kiểm chứng beacon trong bước test, không phải thành phần bắt buộc khi ESP32-S3 đã scan được. |
...
- gửi thông báo Cloud,
- gọi Google Apps Script cho SOS escalation,
- quét thiết bị đáng tin/thiết bị lạ nếu chức năng này được triển khai.

Hệ thống dùng Bluetooth Low Energy (BLE) tích hợp trên ESP32-S3 cho:

- quét thiết bị BLE đáng tin của chủ nhà,
- cập nhật `trusted_ble_present`, `trusted_device_source` hoặc `known_device_present`,
- hỗ trợ giảm báo động thừa khi chủ nhà ở gần khu vực.

Đối với điện thoại dùng app BLE advertiser bên thứ ba, cấu hình demo chuẩn là:
...
Scan Response data:
- Complete Local Name = Mobile_Phone_Number1
...
Vì tên trusted phone nằm trong `Scan Response data`, ESP32-S3 phải dùng active scan. Không yêu cầu Bluetooth pairing, không yêu cầu bấm `CONNECT` và không dùng Bluetooth Settings mặc định của điện thoại để kiểm chứng.
...
| SOS / Escalation | `sos_authority_note` | String | Read & Write | On Change | Parent, Admin | Ghi chú bổ sung của Parent/Admin gửi kèm email authority escalation, ví dụ tình hình hiện tại hoặc nhắc nhở cho công an/cơ quan tiếp nhận |
| SOS / Escalation | `emergency_authority_message_status` | String | Read Only | On Change | Parent, Admin | Trạng thái chuẩn bị/gửi email authority escalation: `IDLE`, `READY`, `SENT`, `FAILED`, `NOT_CONFIGURED` |
...
| WiFi đáng tin / WiFi lạ | `known_device_present` | bool | Read & Write | On Change | Parent, Admin | Có thiết bị đáng tin gần khu vực hay không; trong demo có thể dùng controlled demo input từ Admin Dashboard |
| Thiết bị đáng tin / BLE | `trusted_ble_detection_enabled` | bool | Read & Write | On Change | Parent, Admin | Bật/tắt quét BLE thiết bị đáng tin của chủ nhà |
| Thiết bị đáng tin / BLE | `trusted_ble_present` | bool | Read Only | On Change | Parent, Admin | ESP32-S3 đang phát hiện thiết bị BLE đáng tin gần khu vực |
| Thiết bị đáng tin / BLE | `trusted_device_source` | String | Read Only | On Change | Admin | Nguồn cập nhật thiết bị đáng tin: `BLE_PHONE`, `WIFI_MAC`, `DASHBOARD_INPUT`, `NONE`, `BLE_ERROR` |
| Thiết bị đáng tin / BLE | `trusted_ble_rssi` | int | Read Only | Periodic | Admin | Cường độ tín hiệu BLE gần nhất của thiết bị đáng tin, dùng debug khoảng cách tương đối |
| Thiết bị đáng tin / BLE | `trusted_ble_last_seen_seconds` | int | Read Only | Periodic | Admin | Số giây từ lần cuối ESP32-S3 thấy thiết bị BLE đáng tin |
...
| `emergency_authority_contact` | Không tạo Cloud Variable; contact cơ quan chức năng/công an hoặc contact mô phỏng được cấu hình bảo mật trong Google Apps Script |
| `home_address` | Không tạo Cloud Variable; địa chỉ nhà được cấu hình trong Google Apps Script/config triển khai để tránh hiển thị thông tin nhạy cảm trên dashboard |
...
`known_device_present` là biến bắt buộc để thể hiện logic thiết bị đáng tin. Trong bản demo hoàn chỉnh, biến này ưu tiên được cập nhật từ cơ chế quét BLE thiết bị đáng tin trên ESP32-S3. Hệ thống cũng có thể dùng WiFi/MAC hoặc Admin Dashboard dưới dạng controlled demo input nếu môi trường không cho phép phát hiện ổn định. Dù nguồn cập nhật là cách nào, hệ thống vẫn phải giữ nguyên logic re-arm, `system_armed` và `alarm_status`.
...
### BR-03A. Quy tắc quét BLE để nhận diện thiết bị đáng tin

ESP32-S3 phải được phép dùng Bluetooth Low Energy (BLE) scanning tích hợp để phát hiện điện thoại chủ nhà, BLE tag hoặc ESP32 phụ phát beacon nằm trong whitelist thiết bị đáng tin.

Trong checkpoint 0.6.8, phương án demo chính thức cho điện thoại là dùng app BLE advertiser bên thứ ba. Điện thoại trusted phải phát beacon theo cấu hình tối thiểu sau:
...
Các tên trusted phone mặc định cho nhóm demo là:

```text
Mobile_Phone_Number1
Mobile_Phone_Number2
Mobile_Phone_Number3
Mobile_Phone_Number4
```

Nếu `Complete Local Name` nằm trong `Scan Response data`, firmware ESP32-S3 phải dùng **active BLE scan**. Passive scan không được xem là đạt yêu cầu vì có thể chỉ đọc được `Flags` mà không đọc được tên trusted phone.

Hệ thống không được dùng MAC address BLE của điện thoại làm định danh chính, vì điện thoại có thể dùng cơ chế random/private address. Hệ thống cũng không yêu cầu Bluetooth pairing, không yêu cầu bấm `CONNECT`, và không dùng màn hình Bluetooth Settings mặc định của điện thoại làm bước kiểm thử chính.

Khi `trusted_ble_detection_enabled = true`, hệ thống phải:

1. quét BLE theo chu kỳ nội bộ phù hợp với hiệu năng board,
2. đọc `Complete Local Name` từ advertising data hoặc scan response,
3. so sánh tên phát hiện được với whitelist BLE trusted phone/device,
4. cập nhật `trusted_ble_present = true` nếu thấy thiết bị đáng tin còn trong phạm vi,
5. cập nhật `trusted_device_source = "BLE_PHONE"` nếu app BLE advertiser trên điện thoại là nguồn xác nhận hiện tại,
6. cập nhật `trusted_ble_rssi` và `trusted_ble_last_seen_seconds` cho Admin debug nếu có dữ liệu,
7. cập nhật `known_device_present = true` khi tín hiệu BLE thỏa ngưỡng hiện diện đã cấu hình.

Khi không thấy thiết bị BLE đáng tin trong thời gian timeout nội bộ, hệ thống phải đặt `trusted_ble_present = false` và cho phép logic re-arm theo BR-02 xử lý.

### BR-03B. Quy tắc an toàn cho BLE trusted device

BLE trusted device được dùng để giảm báo động thừa khi chủ nhà ở gần, nhưng không được dùng để tự động xóa một cảnh báo đang active.

Nếu `intrusion_alert`, `sos_alert` hoặc `sabotage_alert` đã active, việc phát hiện lại thiết bị BLE đáng tin chỉ được dùng để ngừng tạo thêm cảnh báo đột nhập mới; cảnh báo hiện tại vẫn phải chờ `reset_alarm`.

Nếu BLE không ổn định trong môi trường demo, Admin được phép tắt `trusted_ble_detection_enabled` hoặc dùng controlled demo input cho `known_device_present`, nhưng phải ghi rõ nguồn là `DASHBOARD_INPUT` trong `trusted_device_source` nếu có thể.

Nếu thư viện BLE cụ thể gây xung đột với Arduino Cloud hoặc không compile trên board profile hiện tại, nhóm phát triển được phép thay đổi thư viện/implementation BLE miễn là vẫn giữ contract ở SRS: active scan khi cần đọc scan response, whitelist trusted phone, cập nhật đúng biến Cloud, và không tự reset cảnh báo active.
...
Phát hiện WiFi/MAC lạ là chức năng bắt buộc trong bản demo hoàn chỉnh, nhưng phải có công tắc `unknown_wifi_detection_enabled` để kiểm soát nhiễu trong môi trường lớp học hoặc khu vực có nhiều điện thoại/laptop. Việc tắt công tắc này không có nghĩa là loại bỏ chức năng khỏi SRS; nó chỉ có nghĩa là tạm thời không dùng tín hiệu MAC lạ trong phiên demo hiện tại.
...
Các tín hiệu cộng điểm phải dùng weighted intrusion score, trong đó tín hiệu xác nhận vật lý mạnh được cộng nhiều điểm hơn tín hiệu bối cảnh.

| Điều kiện | Loại tín hiệu | Điểm |
|---|---|---:|
| `pir_detected = true` | Tín hiệu vật lý mạnh | +2 |
| `object_near = true` | Tín hiệu vật lý mạnh từ HY-SRF05 / HC-SR04-compatible | +2 |
| `light_abnormal = true` | Tín hiệu bối cảnh từ LDR | +1 |
| `night_mode = true` | Tín hiệu bối cảnh theo thời gian | +1 |
| `unknown_wifi_detection_enabled = true` và `unknown_wifi_alert = true` và có ít nhất một tín hiệu vật lý đáng nghi | Tín hiệu hỗ trợ | +1 |
...
Trong đó `pir_detected` và `object_near` là hai tín hiệu xác nhận mạnh. `light_abnormal` và `night_mode` chỉ là tín hiệu bối cảnh hỗ trợ, không nên được xem là bằng chứng độc lập đủ mạnh để kết luận có đột nhập nếu thiếu xác nhận vật lý mạnh.
...
Nếu môi trường demo có quá nhiều MAC lạ hoặc thiết bị dùng MAC randomization làm kết quả phát hiện không ổn định, Admin được phép đặt `unknown_wifi_detection_enabled = false` để bảo vệ luồng demo chính. Khi bật lại, hệ thống phải bắt đầu phiên theo dõi mới theo BR-50.
...
Email xác nhận phải có link hoặc cơ chế xác nhận tương đương, đồng thời cho phép Parent/Admin nhập hoặc chỉnh `sos_authority_note` nếu chức năng này được triển khai ở Apps Script/UI xác nhận.

Nếu ít nhất một Parent/Admin xác nhận qua link trong email, Google Apps Script được phép gửi email escalation đến contact cơ quan chức năng/công an hoặc contact mô phỏng được cấu hình trong Apps Script.

Email escalation phải ghép địa chỉ nhà đã cấu hình, nguồn SOS, thời gian, trạng thái hệ thống và ghi chú bổ sung của Parent/Admin nếu có.

Trong demo, contact này có thể là email phụ của thành viên nhóm và đóng vai trò cơ quan chức năng mô phỏng. Email escalation phải ghi rõ đây là demo nếu nội dung có thể bị hiểu nhầm là cảnh báo thật.

Google Apps Script chỉ xử lý xác nhận và gửi email escalation. Google Apps Script không được tự tắt cảnh báo tại chỗ; việc tắt còi/LED và kết thúc SOS active chỉ do `reset_alarm` từ Parent/Admin thực hiện.
...
- schedule error,
- camera thất bại,
- BLE advertiser phone setup,
- ESP32-S3 active scan thấy trusted phone,
- BLE trusted phone timeout/re-arm.
...
4. không xóa log escalation đã gửi; `emergency_escalation_status` và `emergency_authority_message_status` có thể giữ giá trị gần nhất để Parent/Admin xem lại,
5. đặt `buzzer_on = false`,
6. đặt `led_red_on = false`,
7. đặt `led_green_on = true` nếu không còn cảnh báo active,
8. đặt `send_notification_request = false`,
9. reset cờ nội bộ chống gửi notification lặp cho alert hiện tại,
10. reset cờ nội bộ `auto_photo_captured_for_current_alert`,
11. giữ lại `last_event`, `last_event_type`, `event_counter`,
12. tính lại `system_armed` dựa trên `alarm_enabled`, `known_device_present`, `rearm_delay_seconds`,
13. cập nhật `alarm_status` theo Alarm Status Priority Rule,
14. tự động đặt `reset_alarm = false` sau khi xử lý xong.
...
Trong mọi trường hợp xác nhận hoặc gửi escalation, hệ thống vẫn phải giữ `alarm_status = SOS_ALERT`, `buzzer_on = true`, `led_red_on = true` và `led_green_on = false` cho đến khi Parent/Admin kích hoạt `reset_alarm`.
...
5. không gửi email escalation đến contact cơ quan chức năng/công an hoặc contact mô phỏng,
6. vẫn giữ `alarm_status = SOS_ALERT`, `buzzer_on = true`, `led_red_on = true` và `led_green_on = false` cho đến khi Parent/Admin kích hoạt `reset_alarm`.

### BR-69. Quy tắc cấu hình authority contact và địa chỉ nhà

`emergency_authority_contact` và `home_address` không phải Arduino Cloud Variable trong phiên bản hiện tại.

Contact cơ quan chức năng/công an hoặc contact mô phỏng phải được cấu hình cố định trong Google Apps Script config hoặc cấu hình triển khai bảo mật. Admin Dashboard không cần hiển thị hoặc cho phép chỉnh trực tiếp các giá trị nhạy cảm này.

Trong demo, contact mặc định nên là email phụ của thành viên nhóm và email phải ghi rõ đây là demo. Trong triển khai thật, contact chỉ được cấu hình khi có địa chỉ/email/kênh tiếp nhận hợp lệ và có sự đồng ý của chủ hệ thống.

Nếu `home_address_configured = false` hoặc contact escalation chưa cấu hình hợp lệ, Google Apps Script không được gửi authority escalation và phải trả trạng thái `NOT_CONFIGURED` hoặc `FAILED` cho hệ thống.
...
2. Tính `raw_intrusion_score` theo weighted intrusion score từ `pir_detected` (+2), `object_near` (+2), `light_abnormal` (+1), `night_mode` (+1) và `unknown_wifi_alert` (+1 nếu có tín hiệu vật lý đi kèm).
...
2. Hệ thống tính điểm theo weighted intrusion score:
   - `pir_detected = true` → +2,
   - `object_near = true` → +2,
   - `light_abnormal = true` → +1,
   - `night_mode = true` → +1,
   - WiFi/MAC lạ hợp lệ và có tín hiệu vật lý đi kèm → +1.
...
- Nếu chỉ có `pir_detected = true` và `light_abnormal = true` ở `sensitivity_level = 2`, hệ thống chỉ đạt `intrusion_score = 3`, chưa kích hoạt `intrusion_alert`; hệ thống chỉ cập nhật `threat_level`/log nghi ngờ nếu cần.
- Nếu `intrusion_alert = true`, hệ thống vẫn tiếp tục cập nhật `intrusion_score`, nhưng không tự tắt cảnh báo khi điểm giảm.

#### Acceptance Scenario

Khi `system_armed = true`, `sensitivity_level = 2`, `pir_detected = true`, `object_near = true`, `night_mode = true`, hệ thống phải tính `intrusion_score = 5` và kích hoạt `intrusion_alert = true`.

Khi `system_armed = true`, `sensitivity_level = 2`, `pir_detected = true`, `light_abnormal = true`, `object_near = false` và không có tín hiệu hỗ trợ khác, hệ thống phải tính `intrusion_score = 3` và chưa được kích hoạt `intrusion_alert`.
...
Hệ thống phải hỗ trợ phát hiện thiết bị WiFi/MAC lạ như một dạng cảnh báo sớm, nhưng không được xem đây là báo động đột nhập độc lập. Chức năng này thuộc phạm vi demo hoàn chỉnh, tuy nhiên Admin phải có quyền bật/tắt bằng `unknown_wifi_detection_enabled` để kiểm soát nhiễu môi trường.
...
6. Nếu chụp thất bại, hệ thống đặt `photo_status = "FAILED"`, cập nhật `last_event_type = "photo_capture_failed"` và gửi notification theo Camera Thất bạiure Notification Rule.
...
| SOS Child | Bấm `sos_child` | `SOS_ALERT`, còi/LED đỏ bật và duy trì cho đến khi `reset_alarm`, gửi email xác nhận |
| SOS Adult | Bấm `sos_adult` | `SOS_ALERT`, còi/LED đỏ bật và duy trì cho đến khi `reset_alarm`, gửi email xác nhận |
| SOS Escalation | Bấm link xác nhận trong email | Gửi email authority escalation có địa chỉ nhà và ghi chú bổ sung đến contact được cấu hình; còi/LED đỏ vẫn chưa tắt |
...
| BLE advertiser setup | Điện thoại A bật advertiser `Mobile_Phone_Number1`, điện thoại B dùng BLE scanner kiểm tra | Điện thoại B thấy `Mobile_Phone_Number1` và RSSI hợp lệ; không cần Bluetooth pairing/CONNECT |
...
### FR-16. Nhận diện thiết bị đáng tin bằng BLE scanning

**Trạng thái:** Đã phân rã ở mức rule chính; checkpoint 0.6.8 chốt implementation decision cho điện thoại bằng app BLE advertiser bên thứ ba.

#### Mục đích

Hệ thống phải dùng BLE scanning tích hợp trên ESP32-S3 để nhận diện điện thoại chủ nhà, BLE tag hoặc ESP32 phụ phát beacon đáng tin nhằm giảm báo động thừa khi chủ nhà ở gần khu vực giám sát.

Đối với demo bằng điện thoại, hệ thống không nhận diện bằng MAC address hoặc Bluetooth pairing. Điện thoại trusted phải chạy app BLE advertiser và phát tên đã whitelist.
...
- Người lớn / Phụ huynh
- Admin / Người demo
- Điện thoại trusted chạy app BLE advertiser

#### Cấu hình BLE advertiser chuẩn cho điện thoại demo

| Mục cấu hình | Giá trị yêu cầu |
|---|---|
| Advertising data | `Flags` |
| Scan Response data | `Complete Local Name` |
| Complete Local Name mẫu | `Mobile_Phone_Number1` |
| Scannable | Bật |
| Connectable | Tắt hoặc không bắt buộc; demo ưu tiên tắt |
| Advertising Extension | Tắt để tăng tương thích |
| Anonymous | Tắt |

Danh sách tên trusted phone mặc định:
...
| Biến | Ý nghĩa |
|---|---|
| `trusted_ble_detection_enabled` | Bật/tắt quét BLE thiết bị đáng tin |
| `trusted_ble_present` | Có thiết bị BLE đáng tin đang được phát hiện |
| `trusted_device_source` | Nguồn xác nhận thiết bị đáng tin, ví dụ `BLE_PHONE`, `DASHBOARD_INPUT`, `NONE`, `BLE_ERROR` |
| `trusted_ble_rssi` | Cường độ tín hiệu BLE gần nhất |
| `trusted_ble_last_seen_seconds` | Số giây từ lần cuối thấy thiết bị BLE đáng tin |
| `known_device_present` | Trạng thái tổng hợp cho logic tự disarm/re-arm |

#### Luồng chính

1. Parent/Admin bật `trusted_ble_detection_enabled = true`.
2. Điện thoại trusted bật app BLE advertiser và phát `Complete Local Name` đã whitelist.
3. ESP32-S3 quét BLE theo chu kỳ nội bộ.
4. Nếu tên trusted phone nằm trong `Scan Response data`, ESP32-S3 phải dùng active scan để đọc được tên.
5. Nếu phát hiện thiết bị BLE trong whitelist và tín hiệu đủ điều kiện, hệ thống đặt:
   - `trusted_ble_present = true`,
   - `trusted_device_source = "BLE_PHONE"`,
   - `known_device_present = true`,
   - `trusted_ble_rssi` bằng RSSI gần nhất hoặc mạnh nhất trong cửa sổ scan,
   - `trusted_ble_last_seen_seconds = 0`.
6. Khi `known_device_present = true` và `alarm_enabled = true`, hệ thống áp dụng rule `DISARMED_BY_TRUSTED_DEVICE` nếu không có cảnh báo ưu tiên cao hơn đang active.
7. Khi không còn thấy thiết bị BLE đáng tin trong timeout nội bộ, hệ thống đặt `trusted_ble_present = false` và để BR-02 xử lý re-arm countdown.

#### Ràng buộc

BLE không được tự động reset cảnh báo đang active. Nếu cảnh báo đã xảy ra, Parent/Admin vẫn phải dùng `reset_alarm`.

ESP32-S3 không cần kết nối/pairing với điện thoại. App BLE scanner trên điện thoại khác chỉ dùng để xác nhận beacon đang phát trong bước test ban đầu.

Nếu implementation BLE bằng một thư viện cụ thể không compile cùng Arduino Cloud, nhóm phát triển được phép thay thế implementation bằng thư viện khác hoặc ESP-IDF BLE API, miễn là giữ đúng hành vi yêu cầu trong SRS.

#### Acceptance Scenario

Khi hệ thống đang `ARMED`, `trusted_ble_detection_enabled = true`, và ESP32-S3 active scan phát hiện `Mobile_Phone_Number1`, hệ thống phải cập nhật `trusted_ble_present = true`, `trusted_device_source = BLE_PHONE`, `known_device_present = true`, chuyển `alarm_status = DISARMED_BY_TRUSTED_DEVICE` nếu không có alert ưu tiên cao hơn. Khi điện thoại tắt advertiser hoặc rời đi quá timeout, hệ thống phải đặt `trusted_ble_present = false` và tự re-arm theo BR-02.

Nếu đang `SOS_ALERT`, `INTRUSION_ALERT` hoặc `SABOTAGE_ALERT`, việc phát hiện lại `Mobile_Phone_Number1` không được tắt còi/LED hoặc xóa alert; chỉ `reset_alarm` mới được kết thúc cảnh báo.
...
`unknown_wifi_detection_enabled` phải tồn tại để Admin kiểm soát nhiễu demo. Khi biến này bằng `false`, hệ thống không được dùng MAC lạ để cộng `intrusion_score`; khi bật lại, hệ thống phải reset phiên theo dõi cũ trước khi đếm lại.
...
5. Phát hiện thiết bị đáng tin trong checkpoint 0.6.8 ưu tiên dựa trên BLE scanning tích hợp của ESP32-S3; với điện thoại demo, thiết bị phải chạy app BLE advertiser và phát `Complete Local Name` đã whitelist. WiFi/MAC và controlled demo input chỉ là nguồn phụ hoặc fallback.
6. `known_device_present = true` nghĩa là ít nhất một thiết bị đáng tin được phát hiện gần khu vực giám sát.
7. Trong bản demo, `known_device_present` có thể dùng nguồn BLE thật, WiFi/MAC hoặc controlled demo input từ Admin Dashboard nếu môi trường demo không ổn định.
8. Môi trường lớp học có thể có nhiều thiết bị WiFi/MAC lạ, nên `unknown_wifi_detection_enabled` cần có thể tắt khi demo để tránh nhiễu; khi bật lại, chức năng vẫn phải chạy theo rule WiFi/MAC lạ đã chốt.
9. DS1307 RTC phải được set giờ đúng để lịch tự động chạy đúng.
10. Board `Freenove ESP32-S3 WROOM + Camera OV3660` được xem là phần cứng chính cho bản triển khai hiện tại; nếu đổi sang biến thể ESP32-S3 WROOM/ESP32-S3 CAM khác, cần kiểm tra lại camera sensor, pinout và cấu hình nạp code trước khi code.
11. ESP32-S3 hiện tại có khả năng BLE tích hợp, nên chức năng BLE trusted device không yêu cầu module Bluetooth rời; tuy nhiên điện thoại muốn được nhận diện tự động trong demo phải chạy app BLE advertiser hoặc dùng thiết bị phát BLE đáng tin như BLE tag/ESP32 phụ phát beacon. Nếu điện thoại đặt tên trong `Scan Response data`, firmware phải dùng active scan.
12. Địa chỉ nhà và contact escalation là dữ liệu nhạy cảm, được cấu hình trong Google Apps Script hoặc cấu hình triển khai, không hiển thị trên Child Dashboard.
13. Hệ thống hỗ trợ gửi thông tin SOS nhanh nhất có thể qua email escalation, nhưng không bảo đảm thời gian phản hồi của công an/cơ quan chức năng.
...
|---|---|
| Arduino Cloud | Đồng bộ biến và dashboard |
| WiFi | Đồng bộ Cloud, gọi Google Apps Script, gửi notification và hỗ trợ phát hiện thiết bị đáng tin/lạ khi cần |
| App BLE advertiser trên điện thoại | Phát beacon trusted phone với `Complete Local Name` như `Mobile_Phone_Number1`; không dùng MAC/pairing làm định danh chính |
| BLE tích hợp trên ESP32-S3 | Quét thiết bị BLE đáng tin của chủ nhà để cập nhật `trusted_ble_present` / `known_device_present`; cần active scan nếu tên nằm trong scan response |
| Cảm biến | Phát hiện đột nhập và phá hoại |
| Dịch vụ thông báo | Gửi Telegram hoặc cảnh báo bên ngoài |
| Google Apps Script | Gửi email xác nhận SOS, nhận ACK, ghép địa chỉ nhà/ghi chú bổ sung và gửi authority escalation |
...
5. Gửi cảnh báo tự động đến công an/cứu hỏa/cơ quan khẩn cấp thật khi chưa có xác nhận Parent/Admin hoặc chưa có contact hợp lệ được cấu hình.
...
Ở checkpoint 0.6.5, các yêu cầu lõi phục vụ demo hoàn chỉnh đã được phân rã đủ để bắt đầu code.

Không còn chức năng bắt buộc nào cần phân rã sâu trước khi triển khai. Các điểm cần chú ý hiện tại là kiểm tra pinout/wiring, cấu hình camera OV3660, cấu hình Flash/PSRAM đúng biến thể board, và quyết định nguồn cập nhật thực tế cho `known_device_present`.
...
| Đối chiếu tài liệu wiring/pinout | Verification | Bắt buộc trước khi nạp code để tránh dùng nhầm pinout board cũ |
| Kiểm tra cấu hình Flash/PSRAM và camera OV3660 | Verification | Bắt buộc trước khi code camera và upload sketch |
...
| Báo cháy | Loại bỏ |
| RFID-RC522 | Loại bỏ |
| Board chính hiện tại | `Freenove ESP32-S3 WROOM + Camera OV3660` |
| Camera hiện tại | OV3660 |
| `alarm_enabled` | Chỉ điều khiển phát hiện đột nhập |
| `system_armed` | Trạng thái chống trộm thực tế của khu vực |
| `known_device_present = true` | Tự tắt phát hiện đột nhập cho khu vực |
| Nguồn `known_device_present` trong demo | Có thể từ phát hiện thật hoặc controlled demo input của Admin Dashboard |
...
| WiFi/MAC lạ | Có công tắc `unknown_wifi_detection_enabled`; chức năng vẫn thuộc phạm vi demo hoàn chỉnh |
...
Nếu alarm_enabled = true và known_device_present = true:
    ...
    nguồn known_device_present có thể là phát hiện thật hoặc controlled demo input
...
unknown_wifi_detection_enabled là công tắc kiểm soát nhiễu demo:
    false = không dùng MAC lạ để cộng điểm / gửi alert
    true = bắt đầu hoặc tiếp tục phiên phát hiện theo rule
...
SOS:
    ...
    nếu có xác nhận thì gửi email authority escalation đến contact được cấu hình
    email có địa chỉ nhà, nguồn SOS, thời điểm, trạng thái hệ thống và ghi chú bổ sung
    demo dùng contact mô phỏng; triển khai thật chỉ dùng contact hợp lệ
...
### A.10 Logic BLE Advertiser Phone Scope 0.6.8

```text
Mục tiêu:
    nhận diện điện thoại trusted phone gần khu vực giám sát
    giảm báo động thừa khi người nhà ở gần

Không dùng:
    Bluetooth pairing
    nút CONNECT trong app scanner
    MAC address điện thoại làm định danh chính
    Bluetooth Settings mặc định của điện thoại làm bước kiểm thử chính

Cấu hình điện thoại demo:
    dùng app BLE advertiser bên thứ ba
    Advertising data = Flags
    Scan Response data = Complete Local Name
    Complete Local Name = Mobile_Phone_Number1
    Scannable = true
    Connectable = false hoặc không bắt buộc
    Advertising Extension = false
    Anonymous = false
...
Yêu cầu firmware:
    nếu tên nằm trong Scan Response data:
        phải dùng active BLE scan

Khi thấy trusted phone:
    trusted_ble_present = true
    trusted_device_source = BLE_PHONE
    trusted_ble_rssi = RSSI gần nhất/mạnh nhất
    trusted_ble_last_seen_seconds = 0
    known_device_present = true

Khi mất trusted phone quá timeout:
    trusted_ble_present = false
    trusted_ble_last_seen_seconds tăng theo thời gian
    known_device_present được tính lại theo các nguồn còn lại
    re-arm logic xử lý theo BR-02

Quy tắc an toàn:
    BLE chỉ cập nhật trạng thái thiết bị đáng tin
    BLE không tự reset SOS_ALERT / INTRUSION_ALERT / SABOTAGE_ALERT
    cảnh báo active chỉ kết thúc khi Parent/Admin dùng reset_alarm

Trạng thái implementation hiện tại:
    cấu hình app điện thoại và phone-to-phone scanner đã PASS
    firmware BLE scan trên ESP32-S3 đang là IN PROGRESS cho đến khi compile và test thực tế PASS
```
