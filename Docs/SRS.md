...
> **Phiên bản tài liệu:** 0.6.4
> **Trạng thái:** Bản checkpoint cập nhật quy tắc demo hoàn chỉnh, WiFi/MAC lạ và thiết bị đáng tin
> **Ngày tạo:** 2026-06-26
...
Bản 0.6.4 kế thừa phạm vi nghiệp vụ của checkpoint 0.6.3: tập trung vào chống trộm cho một khu vực giám sát hiện tại là phòng bếp; không đưa lại báo cháy, Flame Sensor, đo nhiệt độ bếp hoặc RFID-RC522 vào phạm vi chính. Thay đổi chính của bản này là làm rõ phạm vi MVP demo hoàn chỉnh, quy tắc kiểm soát WiFi/MAC lạ trong môi trường nhiễu, và cơ chế `known_device_present` cho thiết bị đáng tin.
...
| Trạng thái hiện tại | Bản checkpoint 0.6.4; đã cập nhật board chính sang `Freenove ESP32-S3 WROOM + Camera OV3660`, đồng thời làm rõ MVP demo hoàn chỉnh, WiFi/MAC lạ có kiểm soát và thiết bị đáng tin trong demo |
...
| SOS | SOS Child, SOS Adult, email confirmation/escalation qua Google Apps Script | Không gửi đến cơ quan chức năng thật |
...
| Thiết bị đáng tin | Có logic `known_device_present`, re-arm countdown và `DISARMED_BY_TRUSTED_DEVICE` | Có thể dùng controlled demo input nếu phát hiện thật không ổn định |
...
`known_device_present` là biến bắt buộc để thể hiện logic thiết bị đáng tin. Trong bản demo hoàn chỉnh, biến này có thể được cập nhật bởi cơ chế phát hiện thiết bị đáng tin thật hoặc bởi Admin Dashboard dưới dạng controlled demo input nếu môi trường không cho phép phát hiện ổn định. Dù nguồn cập nhật là cách nào, hệ thống vẫn phải giữ nguyên logic re-arm, `system_armed` và `alarm_status`.
...
7. Trong bản demo, `known_device_present` có thể dùng nguồn phát hiện thật hoặc controlled demo input từ Admin Dashboard nếu thiết bị thật dùng MAC randomization hoặc môi trường demo không ổn định.
...
Ở checkpoint 0.6.4, các yêu cầu lõi phục vụ demo hoàn chỉnh đã được phân rã đủ để bắt đầu code.
