...
> **Phiên bản tài liệu:** 0.7.1
> **Trạng thái:** Bản checkpoint đã đối chiếu với firmware, Arduino Cloud, Telegram và Google Apps Script trong lượt test tích hợp ngày 2026-07-11
> **Ngày tạo:** 2026-06-26
...
Bản 0.7.0 là checkpoint khóa phạm vi cuối trước demo. Phiên bản này chính thức loại BLE trusted phone và quét WiFi/MAC khỏi phạm vi bắt buộc; các biến cũ liên quan có thể tạm tồn tại trong firmware hoặc Arduino Cloud để giữ tương thích nhưng không được xem là chức năng phải triển khai, không được dùng trong logic chống trộm và không đưa lên dashboard demo chính.

Bản 0.7.1 giữ chuỗi xử lý phá hoại hai giai đoạn và ghi nhận cấu hình đã test thực tế:
...
| Heartbeat | ESP32 gửi mỗi 30 giây; Apps Script lưu giờ server của heartbeat cuối |
| Timeout DS-03 | Apps Script nâng cấp khi heartbeat cuối cũ hơn 60 giây sau sabotage; trigger quét mỗi 1 phút nên email thường đến trong khoảng 60–120 giây |
...
| Trạng thái hiện tại | Bản checkpoint 0.7.0; khóa phạm vi demo 6 kịch bản, loại BLE/WiFi scanning, bổ sung heartbeat và critical security compromise sau phá hoại |
...
2. Khi tài liệu demo, dashboard hoặc code cũ mâu thuẫn với SRS v0.7.0, phải ưu tiên SRS v0.7.0.
...
9. BLE trusted phone, `known_device_present`, auto re-arm theo điện thoại và quét WiFi/MAC đã bị loại khỏi phạm vi v0.7.0. Không được tự triển khai lại nếu chưa có phiên bản SRS mới.
10. Các biến BLE/WiFi/re-arm cũ có thể tạm tồn tại để tương thích với Arduino Cloud nhưng phải được xem là legacy — biến cũ không còn tham gia nghiệp vụ.
...
25. Các tài liệu dashboard cũ chỉ là tài liệu tham khảo bố cục; dashboard v0.7.0 phải bỏ widget BLE/WiFi/re-arm và thêm widget heartbeat/critical compromise.
...
29. Các yêu cầu lịch sử đã bị loại vẫn được giữ ID để truy vết nhưng phải có nhãn `Loại khỏi phạm vi v0.7.0`.
...
| ID | Chức năng | Trạng thái v0.7.0 |
|---|---|---|
...
Các biến cũ liên quan BLE/WiFi/re-arm có thể tạm tồn tại trong code hoặc Thing để tránh lỗi tương thích, nhưng không được dùng trong logic nghiệp vụ hoặc dashboard demo.
...
- ESP32-S3 gửi heartbeat mặc định mỗi 10 giây; giá trị có thể cấu hình trong code nhưng phải nằm trong khoảng 5–30 giây cho bản demo.
- Google Apps Script xem heartbeat quá hạn khi không nhận tín hiệu trong 30–60 giây.
...
> Danh sách này là phạm vi v0.7.0. Biến legacy được liệt kê riêng ở mục 5.5.
...
### 5.5 Biến Cloud đã loại bỏ hoặc chuyển sang legacy

Các biến dưới đây không thuộc phạm vi nghiệp vụ v0.7.0. Nếu chúng đã tồn tại trong Thing hoặc code baseline, nhóm có thể giữ tạm để tránh lỗi đồng bộ nhưng phải ẩn khỏi dashboard và không dùng trong logic.

| Biến | Trạng thái |
|---|---|
| `known_device_present` | Legacy; không dùng để tính `system_armed` |
| `trusted_ble_detection_enabled` | Legacy; BLE bị loại |
| `trusted_ble_present` | Legacy |
| `trusted_device_source` | Legacy |
| `trusted_ble_rssi` | Legacy |
| `trusted_ble_last_seen_seconds` | Legacy |
| `rearm_delay_seconds` | Legacy; auto re-arm theo điện thoại bị loại |
| `rearm_countdown_remaining` | Legacy |
| `unknown_wifi_detection_enabled` | Legacy; WiFi scanning bị loại |
| `unknown_wifi_count` | Legacy |
| `unknown_wifi_alert` | Legacy |
| `last_unknown_mac` | Legacy |
...
ESP32-S3 gửi heartbeat đến Google Apps Script mỗi 30 giây trong cấu hình demo đã test. Gửi heartbeat thất bại không được chặn vòng lặp cảm biến quá lâu.

### BR-97. Quy tắc heartbeat timeout

Apps Script chỉ nâng cấp DS-03 khi heartbeat cuối cũ hơn 60 giây sau sabotage. Trigger kiểm tra chạy mỗi phút, nên thời gian phát hiện thực tế có thể nằm trong khoảng 60–120 giây sau heartbeat cuối. Một request lỗi đơn lẻ không đủ để kết luận offline.
...
Nếu heartbeat quá hạn nhưng không có sabotage active/gần đây, Apps Script chỉ ghi `DEVICE_OFFLINE` hoặc `CONNECTION_LOST`; không gửi authority escalation.
...
Dashboard v0.7.0 không hiển thị BLE, WiFi scanning, trusted phone hoặc re-arm countdown.
...
1. ESP32-S3 gửi heartbeat mỗi 30 giây.
2. Apps Script lưu `lastHeartbeatAt`, `deviceId` và trạng thái gần nhất.
3. Nếu heartbeat còn trong giới hạn, trạng thái là online.
4. Nếu heartbeat cuối quá 60 giây sau sabotage, trạng thái được nâng lên critical; do trigger chạy mỗi phút, email đến trong khoảng 60–120 giây.
5. Mất liên lạc không có sabotage chỉ tạo log/trạng thái offline.

#### Acceptance Scenario

Dừng heartbeat khi không có sabotage không được gửi email escalation đến contact mô phỏng.
...
Heartbeat timeout phải có khoảng đệm đủ lớn so với chu kỳ gửi để tránh báo offline do một request lỗi. Cấu hình đã test là heartbeat 30 giây và timeout lớn hơn 60 giây; Apps Script chỉ xử lý timeout sau sabotage active.

### NFR-16. Khả năng truy vết event
...
`rearm_delay_seconds` và `rearm_countdown_remaining` không thuộc nghiệp vụ v0.7.0. Nếu còn trong Thing, chúng phải được ẩn khỏi dashboard và không tác động `system_armed`.
...
Checkpoint 0.7.0 khóa phạm vi 6 kịch bản. Không được tự thêm lại BLE/WiFi scanning, nhận diện khuôn mặt, phần cứng mới hoặc escalation tự động không xác nhận.

### CON-14. Ràng buộc heartbeat

Chu kỳ heartbeat demo hiện cố định 30 giây. Timeout Apps Script hiện là 60 giây và trigger kiểm tra mỗi 1 phút; không thay đổi các giá trị này nếu chưa kiểm thử lại luồng DS-03.
...
14. Mobile app riêng.
15. Retry queue phức tạp khi mạng phục hồi.
...
|---:|---|---|
| 1 | Cập nhật Google Apps Script | Nhận heartbeat, sabotage, critical event; timeout; email xác nhận; chống gửi trùng |
| 2 | Cập nhật firmware | Gửi heartbeat, gửi sabotage event, polling status, fallback khi camera lỗi |
| 3 | Cập nhật Arduino Cloud variables | Thêm nhóm heartbeat/critical; ẩn biến legacy |
| 4 | Cập nhật dashboard | Bỏ BLE/WiFi/re-arm; thêm trạng thái online/critical/escalation |
| 5 | Chuẩn hóa thông báo tiếng Việt | Đồng nhất Serial, Cloud, Telegram và Gmail |
| 6 | Chạy acceptance tests | Chạy đủ 6 kịch bản và test âm |
...
| Heartbeat | Board gửi 30 giây/lần; timeout Apps Script là 60 giây, trigger kiểm tra mỗi 1 phút |
...
Các yêu cầu BLE trusted phone, unknown WiFi/MAC và auto re-arm trong v0.6.x chỉ còn giá trị lịch sử. Không dùng chúng để đánh giá code v0.7.0.
