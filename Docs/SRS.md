...
> **Phiên bản tài liệu:** 0.6.7
> **Trạng thái:** Bản checkpoint cập nhật BLE nhận diện chủ nhà, weighted intrusion score và SOS alarm latch/reset
> **Ngày tạo:** 2026-06-26
...
Bản 0.6.7 kế thừa phạm vi nghiệp vụ của checkpoint 0.6.6: tập trung vào chống trộm cho một khu vực giám sát hiện tại là phòng bếp; không đưa lại báo cháy, Flame Sensor, đo nhiệt độ bếp hoặc RFID-RC522 vào phạm vi chính. Thay đổi chính của bản này là chốt lại DS-06/FR-10 theo logic SOS alarm latch: khi SOS được kích hoạt, còi và LED đỏ phải duy trì trong suốt thời gian chờ Parent/Admin xác nhận, sau khi ACK và kể cả sau khi Google Apps Script gửi escalation thành công; cảnh báo SOS chỉ được kết thúc khi Parent/Admin kích hoạt `reset_alarm`.
...
| Trạng thái hiện tại | Bản checkpoint 0.6.7; giữ board chính `Freenove ESP32-S3 WROOM + Camera OV3660`, bổ sung BLE trusted device scanning, weighted intrusion score và chốt SOS alarm latch/reset cho DS-06/FR-10 |
...
| BLE scanning tích hợp trên ESP32-S3 | Quét điện thoại/tag BLE đáng tin để nhận diện chủ nhà gần khu vực |
...
Trong checkpoint 0.6.5, nguồn phát hiện thiết bị đáng tin ưu tiên là Bluetooth Low Energy (BLE) scanning trên ESP32-S3. Board chính quét điện thoại chủ nhà, BLE tag hoặc ESP32 phụ phát beacon đã được whitelist để cập nhật `trusted_ble_present` và/hoặc `known_device_present`.
...
| Thiết bị đáng tin / BLE | `trusted_device_source` | String | Read Only | On Change | Admin | Nguồn cập nhật thiết bị đáng tin: `BLE`, `WIFI_MAC`, `DASHBOARD_INPUT`, `NONE` |
...
2. so sánh thiết bị phát hiện được với whitelist BLE trusted device,
3. cập nhật `trusted_ble_present = true` nếu thấy thiết bị đáng tin còn trong phạm vi,
4. cập nhật `trusted_device_source = "BLE"` nếu BLE là nguồn xác nhận hiện tại,
5. cập nhật `trusted_ble_rssi` và `trusted_ble_last_seen_seconds` cho Admin debug nếu có dữ liệu,
6. cập nhật `known_device_present = true` khi tín hiệu BLE thỏa ngưỡng hiện diện đã cấu hình.
...
**Trạng thái:** Đã phân rã ở mức rule chính
...
| `trusted_device_source` | Nguồn xác nhận thiết bị đáng tin |
...
2. ESP32-S3 quét BLE theo chu kỳ nội bộ.
3. Nếu phát hiện thiết bị BLE trong whitelist và tín hiệu đủ điều kiện, hệ thống đặt:
   - `trusted_ble_present = true`,
   - `trusted_device_source = "BLE"`,
   - `known_device_present = true`.
4. Khi `known_device_present = true` và `alarm_enabled = true`, hệ thống áp dụng rule `DISARMED_BY_TRUSTED_DEVICE`.
5. Khi không còn thấy thiết bị BLE đáng tin trong timeout nội bộ, hệ thống đặt `trusted_ble_present = false` và để BR-02 xử lý re-arm countdown.
...
Khi hệ thống đang `ARMED` và ESP32-S3 phát hiện BLE trusted device hợp lệ, hệ thống phải cập nhật `trusted_ble_present`, `trusted_device_source`, `known_device_present`, chuyển `alarm_status = DISARMED_BY_TRUSTED_DEVICE`, sau đó tự re-arm khi thiết bị rời đi đủ thời gian cấu hình.
...
5. Phát hiện thiết bị đáng tin trong checkpoint 0.6.5 ưu tiên dựa trên BLE scanning tích hợp của ESP32-S3; WiFi/MAC và controlled demo input chỉ là nguồn phụ hoặc fallback.
...
11. ESP32-S3 hiện tại có khả năng BLE tích hợp, nên chức năng BLE trusted device không yêu cầu module Bluetooth rời; tuy nhiên cần có thiết bị phát BLE đáng tin như điện thoại, BLE tag hoặc ESP32 phụ phát beacon nếu muốn demo tự động thật.
...
| BLE tích hợp trên ESP32-S3 | Quét thiết bị BLE đáng tin của chủ nhà để cập nhật `trusted_ble_present` / `known_device_present` |
...
## Kết thúc bản SRS checkpoint 0.6.5
